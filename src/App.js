...
import { ArrowLeft, BookOpen, Shield, Home, Lock, Zap, ChevronDown, CheckCircle, Circle, Bell, X, LogOut } from 'lucide-react';
...
const Logo = ({ className }) => (
    ...
    >
        <path d="M10 45 H90 V55 H10 Z" fill="#FBBF24" />
        <path d="M5 40 H15 V60 H5 Z" fill="#FBBF24" />
        <path d="M85 40 H95 V60 H85 Z" fill="#FBBF24" />
        <path d="M50 10 C30 10, 20 30, 20 45 H50 Z" fill="#3B82F6" />
        <path d="M50 10 C70 10, 80 30, 80 45 H50 Z" fill="#FBBF24" />
        <path d="M50 10 V45" stroke="white" strokeWidth="2" />
    </svg>
);
...
export default function App() {
    // --- State Management ---
    const [view, setView] = useState('loading'); // loading, login, signup, introduction, dashboard, session, journal, gamePlan, reminders
    ...
    useEffect(() => {
        ...
        const unsubscribeSnapshot = onSnapshot(userDocRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                setGamePlan(data.gamePlan || { purpose: '', affirmations: '', routine: '', resets: '' });
    ...
    const startSession = (week, dayIndex) => {
        ...
        if (status === 'unlocked' && userData && dayIndex === userData.progress.day) {
    ...
    const handleGamePlanChange = async (field, value) => {
        const updatedPlan = { ...gamePlan, [field]: value };
        setGamePlan(updatedPlan);

        const userDocRef = doc(db, 'artifacts', appId, 'users', user.uid);
        try {
            await setDoc(userDocRef, { gamePlan: updatedPlan }, { merge: true });
        } catch (error) {
    ...
    const renderView = () => {
        ...
        switch (view) {
            case 'introduction': return <IntroductionPage onComplete={markIntroAsSeen} />;
            case 'dashboard':
                return <Dashboard 
            ...
            case 'session':
                ...
                return <DailySession 
                        ...
                        />;
            case 'journal': return <JournalView entries={journalEntries} courseData={courseData} />;
            case 'gamePlan':
                 return <GamePlanView 
                            planData={gamePlan} 
                            onPlanChange={handleGamePlanChange} 
...
const DailySession = ({ sessionData, sessionInfo, onComplete, onBack, journalEntry, onJournalChange }) => {
    return (
        <div className="bg-gray-800 rounded-xl p-6 shadow-lg animate-fade-in">
            ...
            <div className="mt-8 text-center">
                <button 
                    onClick={onComplete}
                    disabled={!journalEntry?.text?.trim()}
...
const GamePlanView = ({ planData, onPlanChange, paymentStatus, onUnlockRequest }) => {
    ...
    const GamePlanInput = ({ label, placeholder, value, field, onPlanChange }) => (
        <div>
            <label className="block text-lg font-bold text-yellow-400 mb-2">{label}</label>
            <textarea
                value={value}
                onChange={(e) => onPlanChange(field, e.target.value)}
    ...
    return (
        <div className="animate-fade-in space-y-8">
            ...
            <GamePlanInput label="My Purpose Statement" placeholder="What is your 'Why'?" value={planData.purpose} field="purpose" onPlanChange={onPlanChange} />
            <GamePlanInput label="Core Affirmations" placeholder="What powerful truths will you remind yourself of?" value={planData.affirmations} field="affirmations" onPlanChange={onPlanChange} />
            <GamePlanInput label="Pre-Game Routine" placeholder="What is your mental warm-up?" value={planData.routine} field="routine" onPlanChange={onPlanChange} />
            <GamePlanInput label="In-Game Reset Strategies" placeholder="How will you reset after a mistake or during a timeout?" value={planData.resets} field="resets" onPlanChange={onPlanChange} />
        </div>
    );
};
            ...
            <div className="bg-gray-800 rounded-xl p-8 shadow-2xl w-full max-w-md m-4 border border-yellow-400/30">
                ...
                <div className="space-y-4">
                    {!paymentStatus.gamePlanUnlocked && !paymentStatus.block3 && (
                         <button onClick={() => onPayment('gamePlan')} className="w-full text-left p-4 bg-gray-700 hover:bg-gray-600 rounded-lg transition-colors">
                    ...
                    {!paymentStatus.block2 && (
                        <button onClick={() => onPayment('block2')} className="w-full text-left p-4 bg-gray-700 hover:bg-gray-600 rounded-lg transition-colors">
                     ...
                     {!paymentStatus.block3 && (
                        <button onClick={() => onPayment('block3')} className="w-full text-left p-4 bg-gray-700 hover:bg-gray-600 rounded-lg transition-colors">
                    ...
                    {(!paymentStatus.block2 || !paymentStatus.block3) && (
                         <button onClick={() => onPayment('both')} className="w-full text-left p-4 bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-400 hover:to-orange-400 rounded-lg transition-all text-white shadow-lg">
